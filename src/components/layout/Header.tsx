...
const Header = () => {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-background/98 backdrop-blur-lg shadow-md" : "bg-background/95 backdrop-blur-md"} border-b border-border`}>
      {/* Top bar */}
      ...
      <nav className="container py-0">
        <div className="flex items-center justify-between py-[10px]">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <img src={rentalYachtLogo} alt="Rental Yacht Dubai" className="w-14 h-14 md:w-16 md:h-16 object-contain" />
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map(link => link.hasDropdown ? <NavigationMenu key={link.path}>
                  <NavigationMenuList>
                    <NavigationMenuItem>
                      ...
                      <NavigationMenuContent>
                        <div className="w-[400px] p-4 grid gap-2">
                          ...
                          {tourCategories.map(category => <NavigationMenuLink asChild key={category.path}>
                              <Link to={category.path} className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors group">
                ...
                </NavigationMenu> : <Link key={link.path} to={link.path} className={`text-sm font-medium transition-all px-4 py-2 rounded-lg hover:bg-muted/50
          ...
          {/* Mobile Menu Button */}
          <button className="lg:hidden p-2 rounded-lg hover:bg-muted/50 transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Toggle menu">
            {isMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
        </div>

        {/* Mobile Navigation */}
        {isMenuOpen && <div className="lg:hidden absolute top-full left-0 right-0 bg-background border-b border-border shadow-xl animate-slide-in-right max-h-[calc(100vh-80px)] overflow-y-auto">
            <div className="container py-4 sm:py-6 flex flex-col gap-1 sm:gap-2 px-4">
              {navLinks.map(link => <Link key={link.path} to={link.path} className={`text-base font-medium py-3 px-4 rounded-lg transition-colors touch-target
                    ${isActive(link.path) ? "text-secondary bg-secondary/10" : "text-foreground hover:bg-muted/50 hover:text-secondary"}`} onClick={() => setIsMenuOpen(false)}>
                  {link.name}
                </Link>)}
              ...
              <div className="mt-3 pt-3 sm:mt-4 sm:pt-4 border-t border-border">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {tourCategories.map(category => <Link key={category.path} to={category.path} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors touch-target">
