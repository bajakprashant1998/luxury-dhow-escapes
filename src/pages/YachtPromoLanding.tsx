...
/* ─── Image paths (public folder — NOT bundled into JS) ─── */
const IMG = {
  hero: "/assets/promo/yacht-dubai-skyline.jpg",
  partyDeck: "/assets/promo/yacht-party-deck.jpg",
  interior: "/assets/promo/yacht-luxury-interior.jpg",
  sunset: "/assets/promo/yacht-sunset-couple.jpg",
  aerial: "/assets/promo/yacht-aerial.jpg",
  dining: "/assets/promo/yacht-dining.jpg",
  palm: "/assets/promo/yacht-palm-jumeirah.jpg",
  champagne: "/assets/promo/yacht-champagne-night.jpg",
  pool: "/assets/promo/yacht-pool-deck.jpg",
};
...
const showcaseImages = [
  { img: IMG.interior, label: "Luxury Interiors", sub: "World-class comfort on the water" },
  { img: IMG.partyDeck, label: "Party Deck", sub: "Celebrate with the Dubai skyline" },
  { img: IMG.sunset, label: "Romantic Sunsets", sub: "Unforgettable golden hour moments" },
  { img: IMG.dining, label: "Gourmet Dining", sub: "Fresh seafood & premium BBQ" },
  { img: IMG.aerial, label: "Premium Fleet", sub: "50+ luxury yachts to choose from" },
  { img: IMG.hero, label: "Iconic Views", sub: "Cruise past Dubai's famous landmarks" },
  { img: IMG.palm, label: "Open Waters", sub: "Crystal clear turquoise escapes" },
  { img: IMG.champagne, label: "Night Celebrations", sub: "Toast to the glittering skyline" },
  { img: IMG.pool, label: "Pool & Lounge", sub: "Relax on expansive sun decks" },
];
...
const GoldButton = memo(({ children, onClick, className = "", type = "button", disabled = false, size = "lg" }: {
  ...
  <button
    ...
    onClick={onClick}
    disabled={disabled}
    className={`font-bold text-black rounded-xl transition-all duration-200 hover:brightness-110 active:scale-[0.98] disabled:opacity-50 ${size === "lg" ? "py-4 px-8 text-base md:text-lg" : "py-3 px-6 text-sm md:text-base"} ${className}`}
...
const SectionHeader = memo(({ tag, title, subtitle }: { tag: string; title: string; subtitle?: string }) => (
  <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={fadeUp} className="text-center mb-10 md:mb-14">
...
/* ─── Input class ─── */
const inputCls = "w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm placeholder:text-gray-500 focus:outline-none focus:border-[#d4a853]/50 transition-colors";
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans overflow-x-hidden">
            ...
            <motion.div initial="hidden" animate="visible" variants={stagger}>
              ...
              <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3">
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl font-semibold border border-white/20 hover:border-white/50 hover:bg-white/5 transition-colors text-sm md:text-base"
                >
      ...
      <section className="py-6 md:py-8 border-y border-white/10" style={{ background: "#111" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={stagger} className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[
      ...
      {/* ══════ TESTIMONIALS ══════ */}
      <section className="py-14 md:py-20 px-4 sm:px-6" style={{ background: "#0a0a0a" }}>
        <div className="max-w-6xl mx-auto">
          <SectionHeader tag="Testimonials" title="Trusted by 1000+ Happy Guests" />
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {testimonials.slice(0, 3).map((t) => (
              <motion.div key={t.id} variants={scaleIn} className="rounded-2xl p-5 md:p-6 border border-white/10 hover:border-[#d4a853]/30 transition-colors" style={{ background: "#161616" }}>
      ...
      {/* ══════ EXPERIENCE SHOWCASE ══════ */}
      <section className="py-14 md:py-20 px-4 sm:px-6" style={{ background: "#111" }}>
        <div className="max-w-7xl mx-auto">
          <SectionHeader tag="The Experience" title="Unforgettable Moments on the Water" subtitle="From stunning interiors to breathtaking skyline views — every detail is crafted for your perfect day." />

          {/* Mobile: horizontal scroll */}
          <div className="md:hidden -mx-4 px-4">
            <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 scrollbar-hide">
              {showcaseImages.map((item) => (
                <div key={item.label} className="flex-shrink-0 w-[72vw] snap-center">
                  <div className="relative rounded-2xl overflow-hidden aspect-[4/3]">
                    <img src={item.img} alt={item.label} loading="lazy" decoding="async" className="w-full h-full object-cover" width={400} height={300} />
          ...
          {/* Desktop grid */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="hidden md:grid grid-cols-3 gap-4">
            {showcaseImages.map((item) => (
              <motion.div key={item.label} variants={scaleIn} className="relative rounded-2xl overflow-hidden group aspect-[4/3] cursor-pointer">
                <img src={item.img} alt={item.label} loading="lazy" decoding="async" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" width={400} height={300} />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="font-semibold text-lg">{item.label}</p>
                  <p className="text-sm text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity duration-300">{item.sub}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      <section className="relative h-[35vh] sm:h-[40vh] md:h-[50vh] overflow-hidden">
        <img src={IMG.sunset} alt="Romantic yacht sunset" loading="lazy" decoding="async" className="w-full h-full object-cover" width={1024} height={768} />
      ...
      <section className="py-14 md:py-20 px-4 sm:px-6" style={{ background: "#0a0a0a" }}>
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="grid sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-6">
            {packages.map((pkg) => (
              <motion.div
                key={pkg.name}
                variants={scaleIn}
                className="rounded-2xl overflow-hidden border transition-colors duration-200 hover:border-[#d4a853]/60"
              ...
              >
                ...
                <div className="relative h-40 sm:h-48 md:h-52 overflow-hidden">
                  <img src={pkg.image} alt={pkg.name} loading="lazy" decoding="async" className="w-full h-full object-cover" width={400} height={300} />
                ...
                <div className="p-5 md:p-6">
                  ...
                  {pkg.popular ? (
                    ...
                    <button onClick={scrollToBooking} className="w-full py-3 rounded-xl font-semibold text-sm transition-colors border border-white/10 hover:border-[#d4a853]/50 hover:bg-white/5">
                      Book Now
                    </button>
                  )}
                </div>
      ...
      <section className="py-14 md:py-20 px-4 sm:px-6" style={{ background: "#111" }}>
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }} variants={stagger} className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-5">
            {whyChoose.map((item) => (
              <motion.div key={item.title} variants={scaleIn} className="text-center p-4 md:p-6 rounded-2xl border border-white/5 hover:border-[#d4a853]/30 transition-colors" style={{ background: "#1a1a1a" }}>
      ...
      {/* ══════ URGENCY ══════ */}
      <section className="py-14 md:py-20 px-4 text-center" style={{ background: "linear-gradient(135deg, #1a2d4f, #0d1a2d)" }}>
      ...
      {/* ══════ FINAL BOOKING FORM ══════ */}
      <section id="final-booking" className="py-14 md:py-20 px-4 sm:px-6" style={{ background: "#0a0a0a" }}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden pb-safe">
        <div className="p-3 pt-5" style={{ background: "linear-gradient(to top, #0a0a0a 60%, transparent)" }}>
          <GoldButton onClick={scrollToBooking} className="w-full">Book Your Yacht Now</GoldButton>
        </div>
      </div>
    </div>
  );
